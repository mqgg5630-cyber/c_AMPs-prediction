...
if [ -f code/job.sh ]; then
  echo "== 6. job: code/job.sh"; mkdir -p results/jobs; rm -f results/jobs/job_*.log
  JL="results/jobs/job_$(date '+%Y%m%d-%H%M%S').log"
  if timeout 6h bash code/job.sh > "$JL" 2>&1; then echo "OK: job (exit 0)"; else echo "FAIL: job (exit $?)"; fail=1; fi
  echo "--- job log tail (full log: $JL, pushed back with this round) ---"; tail -n 60 "$JL"
fi
