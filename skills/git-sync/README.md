...
## 12. Linux 本机（v2.9.2-linux 扩展，本仓库新增）

Windows 用 `.ps1`，**Linux / WSL 用同名 `.sh`**，协议完全相同（同一个 `handshake.json`，agent 侧脚本不变）：
...
- 铁律不变：同一分支只准一侧值守 live。Windows 与 Linux 都装了就只 `--register` 一边。
- WSL 里 cron 不自启：`sudo service cron start`，或在 `/etc/wsl.conf` 加 `[boot]\ncommand=service cron start`。
- 本机即 Runner：agent 把命令写进 `code/job.sh` 并 `--request`，`local_check.sh` 第 6 节会在真机执行并把日志回传到 `results/jobs/`。
